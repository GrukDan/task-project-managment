...
export class TaskService {
  ...
  getTaskViewModelById(idTask: number):Observable<TaskViewModel> {
    return this.http.get<TaskViewModel>('/api/tasks',{params:new HttpParams().set('id',idTask.toString())});
  }
  ...
  getTaskViewModelsByTaskExecutor(iduser: number):Observable<TaskViewModel[]> {
    return this.http.get<TaskViewModel[]>('/api/tasks/task-view-model/executor',{params:new HttpParams().set('executor',iduser.toString())})
  }
}
